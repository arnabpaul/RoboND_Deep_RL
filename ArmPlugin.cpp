...
void ArmPlugin::Load(physics::ModelPtr _parent, sdf::ElementPtr /*_sdf*/) 
{
	...
	/
	*/
	//int *ptr = get_value("/gazebo/arm_world/camera/link/camera/image");
  
	//cameraSub = None;
	//gazebo::transport::SubscriberPtr cameraSub = cameraNode->Subscribe("/gazebo/arm_world/camera/link/camera/image", ArmPlugin::onCameraMsg (gazebo.msgs.ImageStamped), this);
	...
	*/
	...
	//collisionSub = None;
	//gazebo::transport::SubscriberPtr collisionSub = collisionNode->Subscribe("/gazebo/arm_world/tube/tube_link/my_contact", ArmPlugin::onCollisionMsg (gazebo.msgs.Contacts), this);
		...
		*/
		...
		|| (strcmp(contacts->contact(i).collision2().c_str(),COLLISION_POINT)==0))
		{
			...
			return;
          	...
          	//cout<<"collision reward_loss"<<endl;
...
bool ArmPlugin::updateAgent()
{
	...
	*/
	...
	/
	*/
	float joint; // TODO - Set joint position based on whether action is even or odd.
	/*float joint = 0.0;
  	if (fmod(action,2)==0.0)
      joint += PI/20;
 	else
...
void ArmPlugin::OnUpdate(const common::UpdateInfo& updateInfo)
{
	...
	if( hadNewState && !newReward )
	{
		...
		*/
      	...
      	//gripper_length=sqrt(pow(gripBBox.max.y-gripBBox.min.y,2)-pow(gripBBox.max.x-gripBBox.min.x,2));
